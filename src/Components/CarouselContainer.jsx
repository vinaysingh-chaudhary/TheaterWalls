...
function CarouselContainer({movieData, loading,endpoint }) {
  ...
  return (
    <div className='w-[100%] h-[100%] relative'>
        <AiOutlineArrowLeft onClick={() =>slidingArrowFun("left")} className='w-[5%] h-[100%] absolute left-0'/> 
        <AiOutlineArrowRight onClick={() => slidingArrowFun("right")} className='w-[5%] h-[100%] absolute right-0'/>


     <div className=' w-[100%] h-[100%] overflow-x-auto flex flex-row items-center gap-3'  ref={crslContainer}>
            {movieData?.map((item) => {
                return( !loading? (
                    <Card 
                    key={item.id} 
                    ...
                    release={item.release_date ?dayjs( item.release_date).format("MMM D, YYYY") : dayjs(item.first_air_date).format("MMM D, YYYY")}
